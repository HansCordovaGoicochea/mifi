...
//            ),
        ...
        switch ($this->display) {
            ...
            case 'view':
                ...
                if (($customer = $this->loadObject(true)) && Validate::isLoadedObject($customer)) {
                    array_pop($this->toolbar_title);
                    $this->toolbar_title[] = $this->trans('Information about customer %name%', array('%name%' => $customer->firstname), 'Admin.Orderscustomers.Feature');
                }
                break;
            case 'add':
            case 'edit':
                ...
                /** @var Customer $customer */
                if (($customer = $this->loadObject(true)) && Validate::isLoadedObject($customer)) {
                    $this->toolbar_title[] = $this->trans('Editing customer %name%', array('%name%' => $customer->firstname), 'Admin.Orderscustomers.Feature');
                } else {
    ...
    public function renderForm()
    {
        ...
        $tipo_documentos = TipoDocumentoLegal::getNameAllTipDoc();
...
//                ),
                array(
                    'type' => 'select',
                    'label' => $this->trans('Tipo Documento', array(), 'AdminGlobal'),
                    'name' => 'id_tipo_documento_legal',
                    'required' => false,
                    'col' => '4',
//                    'default_value' => (int) $this->context->country->id,
                    'options' => array(
                        'query' => $tipo_documentos,
                        'id' => 'id_tipo_documento_legal',
                        'name' => 'nombre',
                    ),
                ),
                array(
...
//                    'required' => true,
                ...
                array(
                    'type' => 'text',
                    'label' => $this->trans('Nombres y Apellidos', array(), 'Admin.Global'),
                    'name' => 'firstname',
//                    'required' => true,
                    'col' => '4',
...
//            );
            $this->fields_form['input'][] = array(
                'type' => 'text',
                'label' => $this->trans('Monto Prestamo Permitido', array(), 'Admin.Orderscustomers.Feature'),
...
//            $id_address = Address::getAddressIdBySupplierId($obj->id);
            $id_address = Address::getFirstCustomerAddressId($obj->id);
